 ...
 * @property {boolean} [banner]          cetak banner ASCII 'WhatsBibz.' saat client dibuat.
 *                                    default: hanya di terminal interaktif (stdout TTY);
 *                                    true = selalu, false = jangan pernah.
...
export async function createBibzWhats(options = {}) {
    ...
    // Banner ASCII 'WhatsBibz.' (Small Slant, pelangi) — hanya terminal interaktif secara
    // bawaan; `banner: true` memaksa cetak (mis. untuk log/daemon), `banner: false` mematikan.
    const wantBanner =
        opts.banner === true || (opts.banner !== false && !!process.stdout.isTTY);
    if (wantBanner) {
        try { printBanner(); } catch {}
    }
