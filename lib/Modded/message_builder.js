...
class Button extends BaseBuilder {
    ...
    async build(jid, { ...options } = {}) {
        // Konten interactive message = hasil toCard() (body/footer/header + nativeFlowMessage).
        // Sebelumnya memakai variabel `message` yang tidak pernah didefinisikan → ReferenceError.
        const payload = await this.toCard();
...
class Carousel extends BaseBuilder {
    ...
    async build(jid, { ...options } = {}) {
        // Konsumsi builder terdaftar lewat .card(cb). Sebelumnya _cardBuilders tidak pernah
        // dipakai di sini → carousel terkirim dengan 0 kartu (diam-diam rusak).
        if (this._cardBuilders && this._cardBuilders.length > 0) {
            const built = [];
            for (const builder of this._cardBuilders) {
                built.push(await builder._card.toCard());
            }
            this._cardBuilders = [];
            this.addCard(built);
        }
