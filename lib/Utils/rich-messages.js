...
export const generateUnifiedResponseContent = (quoted, captured) => {
  if (
    !captured ||
    !Array.isArray(captured.submessages) ||
    !captured.unifiedResponse?.data
  ) {
    throw new TypeError(
      'captured wajib berupa hasil captureUnifiedResponse(msg) yang berisi ' +
        'unifiedResponse.data — contoh: sock.sendUnifiedResponse(jid, quoted, captureUnifiedResponse(m.message))',
    );
  }
  const ctxInfo = buildRichContextInfo(quoted);
  return {
